...
import ImageEditPanel from './playground/ImageEditPanel'
import {
  ImageGenPanel, TtsPanel, SttPanel, EmbeddingPanel, RerankPanel,
} from './playground/ModalityPanels'
...
type Mode = 'chat' | 'image-gen' | 'image-edit' | 'tts' | 'stt' | 'embedding' | 'rerank'
...
const MODE_HINTS: Record<Mode, string> = {
  chat: 'Send a chat completion through the router and see which provider serves it.',
  'image-gen': 'Generate images from a text prompt through the router.',
  'image-edit': 'Edit images with inpainting (paint mask) or extend canvases with outpainting.',
  tts: 'Synthesise speech from text (Cloudflare MeloTTS).',
  stt: 'Transcribe an uploaded audio file (Cloudflare Whisper).',
  embedding: 'Vectorise text into an embedding through the router.',
  rerank: 'Rerank documents against a query (Cohere).',
}
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      <PageHeader
        ...
        actions={
          <>
            <div className="inline-flex flex-wrap rounded-md border bg-card p-0.5 shadow-sm">
              {MODES.map(m => (
                <button
                  key={m.id}
                  onClick={() => setMode(m.id)}
                  className={`px-3 py-1.5 text-sm font-medium rounded-[0.3rem] transition-colors ${
                    mode === m.id
      ...
      {mode === 'chat' ? (
        ...
        <div className="flex-1 overflow-y-auto pr-1">
          {mode === 'image-edit' && <ImageEditPanel />}
          {mode === 'image-gen' && <ImageGenPanel />}
          {mode === 'tts' && <TtsPanel />}
          {mode === 'stt' && <SttPanel />}
          {mode === 'embedding' && <EmbeddingPanel />}
          {mode === 'rerank' && <RerankPanel />}
        </div>
      )}
    </div>
  )
