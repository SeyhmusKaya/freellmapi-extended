...
import type { Platform, KeyStatus } from '@myllm/shared/types.js';
...
export async function checkKeyHealth(keyId: number): Promise<KeyStatus> {
  ...
  try {
    ...
    //
    // Logged at warn (not error) because slow providers (Ollama Cloud, OpenRouter
    // under load) occasionally trip the 10s timeout and recover on next sweep.
    const aborted = err?.name === 'AbortError' || /aborted|timeout/i.test(err?.message ?? '');
    if (aborted) {
      console.warn(`[Health] Key ${keyId} transport timeout (retry in ${CHECK_INTERVAL_MS / 1000}s)`);
    } else {
...
export async function checkAllKeys(): Promise<void> {
  ...
  // Surface expiring keys before they 401 in production. NVIDIA dev keys
  // expire 6 months from issue; warn at <30 days and again at <7 days.
  const expiring = db.prepare(`
    SELECT id, platform, label, expires_at,
  ...
  `).all() as Array<{ id: number; platform: string; label: string; expires_at: string; days_left: number }>;
  for (const k of expiring) {
    const level = k.days_left < 7 ? 'error' : 'warn';
    console[level === 'error' ? 'error' : 'warn'](
      `[Health] key ${k.id} (${k.platform}/${k.label}) expires in ${k.days_left} days (${k.expires_at})`
    );
  }
