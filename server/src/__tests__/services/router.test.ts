...
describe('Router', () => {
  ...
  beforeEach(() => {
    ...
    // Keyless providers (kilo) route without an api_keys row — disable them so
    // the key-based routing assertions here ("no keys → throw", etc.) hold.
    db.prepare("UPDATE models SET enabled = 0 WHERE platform = 'kilo'").run();
    // Reset fallback order to intelligence ranking
    const models = db.prepare('SELECT id, intelligence_rank FROM models WHERE enabled = 1 ORDER BY intelligence_rank ASC').all() as any[];
