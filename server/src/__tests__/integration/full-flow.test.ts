...
async function req(app: Express, method: string, path: string, body?: any) {
  ...
  if (path.startsWith('/v1/')) headers['Authorization'] = `Bearer ${getUnifiedApiKey()}`;
...
describe('Full Integration Flow', () => {
  ...
  beforeAll(() => {
    ...
    // Keyless providers (kilo, pollinations) route WITHOUT an api_keys row, so
    // the "no keys → 429" invariant below would otherwise make a real network
    // call to them. Disable them to keep the integration flow hermetic.
    db.prepare("UPDATE models SET enabled = 0 WHERE platform IN ('kilo','pollinations')").run();
  });
  ...
  it('Step 13: probe requests are NOT written to analytics (probe=true)', async () => {
    const db = getDb();
    await req(app, 'POST', '/api/keys', { platform: 'groq', key: 'gsk_probe_flow', label: 'probe-flow' });
      ...
      vi.spyOn(global, 'fetch').mockImplementation(async (url, init) => {
        ...
        if (u.includes('api.groq.com')) {
          return { ok: false, status: 401, statusText: 'Unauthorized',
            json: () => Promise.resolve({ error: { message: 'Invalid API Key' } }) } as any;
        }
        return origFetch(url, init);
      });
    const origFetch = global.fetch;

    // Baseline: a NORMAL failing request DOES write at least one row.
    db.prepare('DELETE FROM requests').run();
    db.prepare('DELETE FROM cooldowns').run();
    groqErr();
    await req(app, 'POST', '/v1/chat/completions', {
      model: 'llama-3.3-70b-versatile', no_cascade: true,
      messages: [{ role: 'user', content: 'ping' }],
    });
    vi.restoreAllMocks();
    const normal = db.prepare('SELECT COUNT(*) AS c FROM requests').get() as { c: number };
    expect(normal.c).toBeGreaterThan(0);

    // Probe: identical failing request with probe=true writes NOTHING.
    db.prepare('DELETE FROM requests').run();
    db.prepare('DELETE FROM cooldowns').run();
    groqErr();
    await req(app, 'POST', '/v1/chat/completions', {
      model: 'llama-3.3-70b-versatile', no_cascade: true, probe: true,
      messages: [{ role: 'user', content: 'ping' }],
    });
    vi.restoreAllMocks();
    const probe = db.prepare('SELECT COUNT(*) AS c FROM requests').get() as { c: number };
